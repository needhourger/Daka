...
from .forms import AccountCenterForm
from .forms import ResetPasswordForm
from .forms import AccountManagerAdminForm
from .forms import RecordMakeUpForm
from .forms import AddUserAdminForm
from .forms import AddRecordAdminForm
...
def account_manager_admin(id=-1):
    users = User.query.filter(~User.username.in_(
        current_app.config["ADMIN_LIST"])).all()
    user = User.query.get(id)
    if user is None:
        user = User.query.filter(~User.username.in_(
            current_app.config["ADMIN_LIST"])).first()
...
def clock_in():
    if not current_user.is_clock_in():
        record = Record(date=datetime.now(),
                        clock_in_time=datetime.now(), user_id=current_user.id)
...
def record():
    ...
    worktime = Record.query.filter_by(user=user).with_entities(
        func.sum(Record.hours)).first()[0]
    records = Record.query.filter_by(
        user=user, year=year, month=month).order_by(Record.date.desc()).all()
...
def add_record(id):
    ...
    if form.validate_on_submit():
        record = Record(
            date=form.date.data,
            clock_in_time=form.clock_in.data,
            clock_out_time=form.clock_out.data,
            clock_out=True, user_id=id
        )
        db.session.add(record)
        db.session.commit()
        flash("记录添加成功")
